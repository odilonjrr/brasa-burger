(() => {
  ...
  async function adminHydrate(){ await init(); if(!client) return null; const role=await getStaffRole(); if(!role) throw new Error('Usuário sem permissão administrativa.'); const [orders,products,categories,addons,coupons,areas,banners]=await Promise.all([db().from('orders').select('*,order_items(*,order_item_addons(*))').order('created_at',{ascending:false}).limit(100),db().from('products').select('*,categories(name)').order('display_order'),db().from('categories').select('*').order('display_order'),db().from('addon_groups').select('*,addon_options(*)').order('name'),db().from('coupons').select('*').order('created_at',{ascending:false}),db().from('delivery_areas').select('*').order('name'),db().from('banners').select('*').order('display_order')]); for(const result of [orders,products,categories,addons,coupons,areas,banners]) if(result.error) throw result.error; return {role,orders:orders.data||[],products:products.data||[],categories:categories.data||[],addons:addons.data||[],coupons:coupons.data||[],areas:areas.data||[],banners:banners.data||[]}; }
  ...
  async function adminUpdateOrderStatus(id,status){ await init(); if(!client) return; const {error}=await db().from('orders').update({status}).eq('id',id); if(error) throw error; }
  window.BrasaSupabase={init,db,isConfigured:()=>configured,session,user,signIn,signUp,signOut,getStaffRole,listProducts,listCategories,listAreas,createOrder,getMyOrders,adminHydrate,adminUpsert,adminDelete,adminUpdateOrderStatus};
})();